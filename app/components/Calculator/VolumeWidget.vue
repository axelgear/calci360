...
import Shape3DVisual from './shapes/Shape3DVisual.vue'
import UnitInput from './UnitInput.vue'
import { length } from './calculators/base/length'
import { deriveArea, deriveVolume } from './calculators/base/derived'
...
const L = length
const volumeUnits: VolumeUnitOption[] = [
  /* Metric */
  ...
  { id: 'milliliter', name: 'Milliliter', symbol: 'mL', toBase: deriveVolume(L.centimeter) },
  ...
  /* Imperial */
  { id: 'cubic-inch', name: 'Cubic Inch', symbol: 'in³', toBase: deriveVolume(L.inch) },
  ...
  /* US Liquid */
  { id: 'us-gallon', name: 'US Gallon', symbol: 'gal', toBase: 0.003785411784 },
  { id: 'us-quart', name: 'US Quart', symbol: 'qt', toBase: 0.000946352946 },
  { id: 'us-pint', name: 'US Pint', symbol: 'pt', toBase: 0.000473176473 },
  { id: 'us-cup', name: 'US Cup', symbol: 'cup', toBase: 0.0002365882365 },
  { id: 'us-fluid-ounce', name: 'US Fluid Ounce', symbol: 'fl oz', toBase: 0.0000295735295625 },
  /* Imperial */
  { id: 'imp-gallon', name: 'Imperial Gallon', symbol: 'gal (imp)', toBase: 0.00454609 },
]

/* Selected volume output unit */
const selectedVolumeUnit = ref('cubic-meter')

/* Default volume unit - Cubic Meter */
const defaultVolumeUnit: VolumeUnitOption = { id: 'cubic-meter', name: 'Cubic Meter', symbol: 'm³', toBase: deriveVolume(L.meter) }

const currentVolumeUnit = computed((): VolumeUnitOption => {
  const found = volumeUnits.find((u: VolumeUnitOption) => u.id === selectedVolumeUnit.value)
  return found ?? defaultVolumeUnit
})

/* ========== Area Units for Surface Area Output ========== */
...
const areaUnits: AreaUnitOption[] = [
  ...
  /* Land units */
  { id: 'acre', name: 'Acre', symbol: 'acre', toBase: 4046.8564224 },
...
/* Track cube input syncing to avoid loops and remember last edited field */
...
function fromMeters(val: number, unitId: string): number {
  ...
  const factor = unit ? unit.toBase : 1
  return val / factor
}
...
/* Keep cube inputs (side, face diagonal, space diagonal) in sync */
function syncCubeInputs(sourceId?: string) {
  if (selectedShapeId.value !== 'cube') return
...
/* Calculate volume in base unit (m³) */
const calculatedVolumeBase = computed(() => {
  if (!selectedShape.value) return 0
  try {
    const valuesInMeters = getValuesInMeters()
...
const calculatedVolume = computed(() => {
  if (!currentVolumeUnit.value) return calculatedVolumeBase.value
  /* Convert from m³ to selected unit */
  return calculatedVolumeBase.value / currentVolumeUnit.value.toBase
})

/* Format volume for display */
const formattedVolume = computed(() => {
  const val = calculatedVolume.value
...
/* Surface area (only if shape defines surface-area additional result) */
const surfaceAreaBase = computed(() => {
  const shape = selectedShape.value
  if (!shape?.additionalResults) return 0
  const surface = shape.additionalResults.find(r => r.id === 'surface-area')
  if (!surface) return 0
  try {
    const valuesInMeters = getValuesInMeters()
    return surface.calculate(valuesInMeters)
  } catch {
    return 0
  }
})
...
const surfaceArea = computed(() => {
  if (!currentAreaUnit.value) return surfaceAreaBase.value
  return surfaceAreaBase.value / currentAreaUnit.value.toBase
})

const formattedSurfaceArea = computed(() => {
  const val = surfaceArea.value
  if (!Number.isFinite(val) || val < 0) return '—'
  if (val === 0) return '0'
  if (val < 0.0001 || val >= 1e9) return val.toExponential(4)
  return val.toLocaleString('en-US', { maximumFractionDigits: 4 })
...
const allConversions = computed(() => {
  const baseVolume = calculatedVolumeBase.value
  if (!Number.isFinite(baseVolume) || baseVolume <= 0) return []
  
  return volumeUnits.map(unit => {
    const converted = baseVolume / unit.toBase
...
async function copyResult() {
  try {
    await navigator.clipboard.writeText(calculatedVolume.value.toString())
    copied.value = true
...
/* Surface area unit dropdown state */
const areaDropdownOpen = ref(false)
const areaSearch = ref('')
const areaDropdownRef = ref<HTMLElement | null>(null)

/* Filtered volume units based on search */
const filteredVolumeUnits = computed(() => {
  if (!outputSearch.value.trim()) return volumeUnits
  const query = outputSearch.value.toLowerCase()
  return volumeUnits.filter(u => 
...
const filteredAreaUnits = computed(() => {
  if (!areaSearch.value.trim()) return areaUnits
  const query = areaSearch.value.toLowerCase()
...
function selectOutputUnit(unitId: string) {
  selectedVolumeUnit.value = unitId
  outputDropdownOpen.value = false
  outputSearch.value = ''
}

/* Select surface area unit */
function selectAreaUnit(unitId: string) {
...
/* Close area dropdown on click outside */
onClickOutside(areaDropdownRef, () => {
  areaDropdownOpen.value = false
  areaSearch.value = ''
})
</script>
    ...
    <div class="calculator-body">
      ...
      <div class="visual-section">
        <Shape3DVisual
          ...
          :volume-unit="currentVolumeUnit.symbol"
        />
      </div>
      ...
      <div class="input-section">
        ...
        <div class="result-section">
          <div class="result-box">
            ...
            <div class="result-value">
              ...
              <div class="output-unit-selector" ref="outputDropdownRef">
                ...
                >
                  <span class="unit-symbol">{{ currentVolumeUnit.symbol }}</span>
                ...
                <Transition name="dropdown-slide">
                  <div v-if="outputDropdownOpen" class="output-dropdown">
                    ...
                    <div class="dropdown-options">
                      <button
                        v-for="unit in filteredVolumeUnits"
                        :key="unit.id"
                        type="button"
                        class="dropdown-option"
                        :class="{ active: unit.id === selectedVolumeUnit }"
                        @click="selectOutputUnit(unit.id)"
                      >
                        <span class="option-name">{{ t(unit.name) }}</span>
                        <span class="option-symbol">{{ unit.symbol }}</span>
                        <span v-if="unit.id === selectedVolumeUnit" class="material-icons check">check</span>
                      </button>
                    </div>
                  </div>
                </Transition>
          ...
          <!-- Surface Area card -->
          <div v-if="surfaceAreaBase > 0" class="result-box">
            <div class="result-label">
              <span class="material-icons">grid_on</span>
              <span>{{ t('Surface Area') }}</span>
            </div>
            <div class="result-value">
              <span class="value">{{ formattedSurfaceArea }}</span>
              <!-- Surface area unit selector -->
              <div class="output-unit-selector" ref="areaDropdownRef">
                <button
                  type="button"
                  class="unit-btn"
                  :class="{ open: areaDropdownOpen }"
                  @click="areaDropdownOpen = !areaDropdownOpen"
                >
                ...
                <Transition name="dropdown-slide">
                  <div v-if="areaDropdownOpen" class="output-dropdown">
                    <div class="dropdown-search">
                      <span class="material-icons">search</span>
                      <input
                        v-model="areaSearch"
    ...
    <div class="conversions-section">
      ...
      <div class="conversions-grid">
        <div
          ...
          :class="{ active: conv.id === selectedVolumeUnit, copied: copiedId === conv.id }"
